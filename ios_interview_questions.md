...
Là kỹ thuật giúp tách một class độc lập với các biến phụ thuộc. 
Để implement Dependency Injection, ta cần tuân thủ một số quy tắc:
* Các class không phụ thuộc trực tiếp lẫn nhau mà sẽ liên kết thông qua interface
* Việc khởi tạo class cung cấp  sẽ do interface thực hiện thay vì class phụ thuộc

Ví dụ về dependency injection:
* **detail interface**:
```sh
protocol DetailNavigatorType {
    func toVideoScreen(video: Video)
    func back()
}
```
* **detailNavigator**:
...
```
* **detailViewModel** chứa navigator (Là một class khác, cần các function để hoạt động -> ViewModel **depends** on  DetailNavigator):
```sh
struct DetailViewModel {
    private let navigator: DetailNavigatorType
    
    init(navigator: DetailNavigatorType) {
        self.navigator = navigator
    }
    ...
    // Sự kiện navigate tới video được kích hoạt từ  view gửi qua
    navigator.toVideoScreen(video: Video)
    ...
}
```
